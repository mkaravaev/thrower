defmodule Thrower.AttackMode.ClosestFirstTest do
  use ExUnit.Case
  ...
  test "should exclude positions with good characters" do
    ...
    assert %{position: %{x: 0, y: 40}} = AttackMode.ClosestFirst.apply(entries)
  end

  test "should return closest radar entry" do
    entries = [
      %RadarEntry{
        position: %RadarEntry.Position{x: 0, y: 40},
        villains: [
    ...
    assert %{position: %{x: 0, y: 20}} = AttackMode.ClosestFirst.apply(entries)
  end
end
